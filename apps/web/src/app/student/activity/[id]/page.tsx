...
type ViewPayload = { version: string; session_id: number; level_id: number; item_id: number; stable_key: string; kind: string; interaction_type: Interaction; round: Round; retry: boolean; attempts_used: number; max_attempts: number; step: { id: number; order_index: number; expected_reading_text?: string | null; required_selection_count: number; options: Option[]; assets: Asset[]; media_gaps: Array<{ status?: string; semantic_text?: string }> }; assets: Asset[] };
...
export default function StudentActivityPage() {
  ...
  const targetCount = view?.step.required_selection_count ?? 0;
  ...
  const toggleOption = (id: number) => {
    ...
    if (MULTI.has(interaction)) return setSelected((current) => {
      if (current.includes(id)) return current.filter((value) => value !== id);
      if (targetCount > 0 && current.length >= targetCount) return current;
      return [...current, id];
    });
    if (ORDER.has(interaction)) setSelected((current) => {
  ...
  const startRecording = async () => {
    ...
    try {
      ...
      recorder.onstop = () => { const blob = new Blob(chunksRef.current, { type: recorder.mimeType || "audio/webm" }); setAudioBlob(blob); setAudioUrl((current) => { if (current) URL.revokeObjectURL(current); return null; }); setAudioUrl(URL.createObjectURL(blob)); stream.getTracks().forEach((track) => track.stop()); };
  ...
  return <div className={styles.page} dir="rtl" data-testid="activity-session" data-phase="active">
    ...
    <main className={styles.shell}><section className={styles.card}>
      ...
      <div className={styles.contentColumn}>
        ...
        {!hasMediaGap && interaction !== "memory_sequence" && !ORDER.has(interaction) && !READ.has(interaction) && isImageChoice && <div className={styles.imageOptions}>{imageOptions.map((asset) => { const id = Number(asset.option_id); const chosen = selected.includes(id); const capped = MULTI.has(interaction) && selectionLimitReached && !chosen; return <button key={asset.asset_id} className={`${styles.imageOption} ${chosen ? styles.optionSelected : ""}`} onClick={() => toggleOption(id)} disabled={capped}>{chosen && <span className={styles.selectedMark}><Check size={18} /></span>}<Image src={asset.url} alt={asset.semantic_text || "خيار مصور"} width={220} height={150} unoptimized /><span className={styles.imageLabel}>{asset.semantic_text || options.find((option) => option.id === id)?.text}</span></button>; })}</div>}
        {!hasMediaGap && interaction !== "memory_sequence" && !ORDER.has(interaction) && !READ.has(interaction) && !isImageChoice && <div className={styles.options}>{options.map((option) => { const chosen = selected.includes(option.id); const capped = MULTI.has(interaction) && selectionLimitReached && !chosen; return <button key={option.id} className={`${styles.option} ${chosen ? styles.optionSelected : ""}`} onClick={() => toggleOption(option.id)} disabled={capped}>{chosen && <span className={styles.selectedMark}><Check size={18} /></span>}{option.text}</button>; })}</div>}
