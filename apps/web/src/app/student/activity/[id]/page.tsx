...
export default function StudentActivityPage() {
  ...
  const loadCurrent = useCallback(async () => {
    ...
    setError("");
    try {
      // Execution endpoint prepares/advances the adaptive attempt. Its legacy
      // presentation fields are intentionally ignored by this renderer.
  ...
  const submitStructured = async (declaredMediaGapSkip = false) => {
    ...
    try {
      const response = await fetch(`/api/activities/session/${sessionId}/attempt/${view.item_id}/submit`, {
        ...
        body: JSON.stringify({ step_id: view.step.id, selected_option_ids: selected, hint_used: view.retry, elapsed_seconds: elapsed, declared_media_gap_skip: declaredMediaGapSkip }),
      });
  ...
  const uploadReading = async () => {
    ...
    try {
      ...
      const submit = await fetch(`/api/activities/session/${sessionId}/attempt/${view.item_id}/submit`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Idempotency-Key": idempotencyKey("answer") },
        body: JSON.stringify({ step_id: view.step.id, audio_storage_key: uploaded.audio_storage_key, audio_file_size: uploaded.audio_file_size, audio_mime_type: uploaded.audio_mime_type, audio_duration_seconds: recordingSeconds, elapsed_seconds: elapsed }),
      });
      const result: SubmitResult = await submit.json().catch(() => ({} as SubmitResult));
    ...
    <div className={styles.page} dir="rtl" data-testid="activity-session" data-phase="active">
      <header className={styles.header}>
        <div className={styles.headerInner}>
          ...
          <button className={styles.exit} type="button" onClick={() => router.push("/student")}><LogOut size={21} /><span>رجوع</span></button>
        </div>
      </header>

      <div className={styles.progressPanel}>
        <div className={styles.progressTop}>
          <span className={styles.assessmentBadge}>{label}</span>
          <span className={styles.progressCount}>{progress?.completed_items ?? 0} من {progress?.total_items ?? 10}</span>
        </div>
        <div className={styles.progressTrack}><div className={styles.progressFill} style={{ width: `${Math.max(percent, 2)}%` }} /></div>
      </div>

      <main className={styles.shell}>
        <section className={styles.card}>
          <div className={styles.progressTop} style={{ position: "relative", zIndex: 2, marginBottom: 0 }}>
            <div className={styles.skillChip}><Target size={19} />{view.round.skill}</div>
            <span className={styles.assessmentBadge}>الجولة {view.round.round_number} من {view.round.round_total}</span>
          </div>
          ...
          <div className={styles.contentColumn}>
              ...
              memoryPreview ? (
                <>
                  <div className={styles.imageOptions}>
                    {imageOptions.map((asset, index) => <div key={asset.asset_id} className={styles.imageOption}><span className={styles.selectedMark}>{index + 1}</span><Image src={asset.url} alt={asset.semantic_text || `الصورة ${index + 1}`} width={220} height={150} unoptimized /><span className={styles.imageLabel}>{asset.semantic_text}</span></div>)}
                  </div>
                  <div className={styles.inlineActions} style={{ marginTop: 16 }}><button className={styles.primary} type="button" onClick={() => setMemoryPreview(false)}>التالي</button></div>
                </>
              ) : (
                <>
            ...
            {!hasMediaGap && interaction !== "memory_sequence" && (interaction === "sequence" || interaction === "build_word") && (
              <>
                <div className={styles.sequenceBoard}>{selected.length === 0 ? <span className={styles.sequenceHint}>ابدأ بالعنصر الأول ثم أكمل بالترتيب.</span> : selected.map((id, index) => <span className={styles.sequenceChip} key={id}><span className={styles.number}>{index + 1}</span>{options.find((option) => option.id === id)?.text}</span>)}</div>
                {sequenceImagesComplete && interaction !== "build_word" ? <div className={styles.imageOptions}>{imageOptions.filter((asset) => !selected.includes(Number(asset.option_id))).map((asset) => <button key={asset.asset_id} className={styles.imageOption} onClick={() => toggleOption(Number(asset.option_id))}><Image src={asset.url} alt={asset.semantic_text || "عنصر ترتيب"} width={220} height={150} unoptimized /><span className={styles.imageLabel}>{asset.semantic_text}</span></button>)}</div> : <div className={styles.options}>{options.filter((option) => !selected.includes(option.id)).map((option) => <button key={option.id} className={styles.option} onClick={() => toggleOption(option.id)}>{option.text}</button>)}</div>}
              </>
            )}
